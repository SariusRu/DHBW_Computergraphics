...
public class DiamondSquareAlg
{
   ...
   // height map two dimensional value storage
    ...
    void diamondSquare(int x, int y, int width) {
        ...
        // Initiates for Diamond-Step:     
        diamondStep(0, 0, width, 1);
    }

    // Performs the diamond-step of the Diamond-Square Algorithm
    void diamondStep(int x, int y, int width, int stepNumber) {
        ...
        // Calculate the average value for center & add random value:
        avg = avg / 4;
        //avg += (Random.Range(-avg, 1 - avg) * 1/Mathf.Pow(stepNumber,2));
        avg += (Random.Range(-avg, 1 - avg) * 1/Mathf.Pow(stepNumber,2));
        ...
        // Make Square-Step
        squareStep(x, y, width, stepNumber);
    }

    // Performs the square-step of the Diamond-Square Algorithm
    void squareStep(int x, int y, int width, int stepNumber) {
        
        // Calculate Average-Value for ...
        // ... Top-Edge Midpoint: 
        diamondAvg(( x + (width-1)/2), y, (width-1)/2, stepNumber);
        // ... Left-Edge Midpoint: 
        diamondAvg(x, ( y + (width-1)/2), (width-1)/2, stepNumber);
        // ... Right-Edge Midpoint: 
        diamondAvg(( x + (width-1)), ( y + (width-1)/2), (width-1)/2, stepNumber);
        // ... Bottom-Edge Midpoint:
        diamondAvg(( x + (width-1)/2), ( y + (width-1)), (width-1)/2, stepNumber);

        // Break condition recursion
        if(width-1 <= 2) return;

        // Recursivly perform Diamond-Step for ...
        // ... Upper-Left Corner:
        diamondStep(x,y,(width+1)/2,stepNumber+1);
        // ... Upper-Right Corner:
        diamondStep(x+((width-1)/2),y,(width+1)/2,stepNumber+1);
        // ... Lower-Left Corner:
        diamondStep(x,y+((width-1)/2),(width+1)/2,stepNumber+1);
        // ... Lower-Right Corner:
        diamondStep(x+((width-1)/2),y+((width-1)/2),(width+1)/2,stepNumber+1);
        
    }

    // Calcuate the average value for the Edge-Midpoints of the Square-Step
    void diamondAvg(int x, int y, int size, int stepNumber) {
